...
def check_loader_update():
    ...
    logger.info("Проверка версии загрузчика...")
...
def download_loader_update() -> bool:
    """
    ...
    try:
        try:
            ...
            if check_loader_update():
                logger.info("Найдена информация об обновлении loader. Попытаюсь скачать.")
                if download_loader_update():
                    return
        except Exception as e:
            logger.debug(f"Ошибка при проверке обновлений loader: {e}")
        try:
            need_download = check_client()
            if need_download:
                ok = download_client()
                if not ok:
                    logger.error(f"Не удалось скачать клиент. Повторим через {CHECK_INTERVAL}s.")
                    time.sleep(CHECK_INTERVAL)
                    return
        except Exception as e:
                    ...
                    if Path(SAVE_CLIENT_PATH).exists():
                        ...
                        if local_hash != server_hash:
                            ...
                            if download_client():
                                ...
                                client_runner.start()
                if check_loader_update():
                    logger.info("Доступно обновление loader. Попытаюсь обновить.")
                    if download_loader_update():
                        return
            except Exception as e:
